...
void AudioHandle::Impl::InternalCallback(int32_t* in, int32_t* out, size_t size)
{
    ...
    if(audio_handle.callback_)
    {
        ...
        for(size_t i = 0; i < size; i += 2)
        {
            ...
            if(chns > 2)
            {
                fin[2][i / 2] = s242f(audio_handle.buff_rx_[1][offset + i])
                                * audio_handle.postgain_recip_;
                fin[3][i / 2] = s242f(audio_handle.buff_rx_[1][offset + i + 1])
                                * audio_handle.postgain_recip_;
            }
        }
        //         break;
        ...
        for(size_t i = 0; i < size; i += 2)
        {
            ...
            if(chns > 2)
            {
                audio_handle.buff_tx_[1][offset + i]
                    = f2s24(fout[2][i / 2] * audio_handle.output_adjust_);
                audio_handle.buff_tx_[1][offset + i + 1]
                    = f2s24(fout[3][i / 2] * audio_handle.output_adjust_);
            }
        }
        //         break;
        //     case SaiHandle::Config::BitDepth::SAI_32BIT:
