...
class UiPage
{
  public:
    ...
    /** Called when any button is held beyond one of the optionally configured long
     *  press thresholds (via ButtonMonitor)
     *
     *  @param buttonID         The ID of the affected button.
     *  @param msHeld           How long the button was held in ms
     *  @returns    false, if you want the event to be passed on to the page below.
     */
    virtual bool OnButtonLongPressed(uint16_t buttonID, uint32_t msHeld)
    {
        (void)(buttonID); // silence unused variable warnings
        (void)(msHeld);   // silence unused variable warnings
        return true;
    }
